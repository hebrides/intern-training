...
4) Kubernetes (set up a "cluster" of containers across multiple "nodes" - VPSs or bare metal machines — so that various application components hosted in the containers (DBs, MQs, clients, servers, etc.) scale and change services automatically in response to network demand): https://helm.sh
